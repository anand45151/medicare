...
const DoctorContactCard = ({ doctor, index }) => {
    const { name, specialty, avgRating, totalRating, photo, totalPatients, hospital } = doctor;

    return (
        <div className="flex items-center bg-white border rounded-lg shadow-md p-4 mb-4">
            ...
            <div className="ml-4 flex-1">
                <h2 className="text-xl font-bold text-headingColor">{name}</h2>
                <p className="text-base text-textColor mt-1">{specialty}</p>
                <div className="mt-4">
                    ...
                    <p className="text-sm text-textColor">Patients: {totalPatients}</p>
                    <p className="text-sm text-textColor">Hospital: {hospital}</p>
                </div>
            </div>
            <div className="flex flex-col items-center ml-auto">
                <Link
                    to="/doctors"
                    className="w-10 h-10 rounded-full border border-solid border-[#181A1E] flex items-center justify-center group hover:bg-primaryColor hover:border-none mb-2"
                >
